...
abstract class templateFactory {
    ...
    $registry->router->getModule(null);


		if($registry->site == 'admin') {
      if($registry->router->module() == 'index') {
        $tpl = access::check('main', $registry->admin_view_privilege) ? "admin_home_private" : "admin_public";
      }
      else {
      ...
      if($registry->router->module() == 'borromeo') {
        $tpl = 'page_borromeo';
      }
      elseif($registry->user->id) $tpl = $registry->isHome ? "home_private" : "page_private";
