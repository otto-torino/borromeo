...
function validateForm(formObj) {
    ...
    var felement = felements[felements.length-1];
        ...
        felement.value = CKEDITOR.instances[felement_name].getData();
		}
...
function chargeEditor(selector, stylesheets, keyt) {
	dojo_textareas = [];
	dojo.ready(function(){
  		...
  		if(textareas && textareas.length){
			...
			for(var i=0; i<textareas.length; i++) {
				...
				var key = typeOf(keyt) == 'null' ? $(textarea).getParents('form')[0].get('name')+'_'+$(textarea).get('id') : keyt;
